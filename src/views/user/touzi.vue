  ...
  <div class="touzi-content">
    ...
    <div v-for="(item, key) in data" :key="key" class="box">
      ...
      <div style="width: 40%" class="left">
        <div class="desc"><span>投资金额: </span> {{ item.money }}元</div>
        <div class="desc"><span>预期收益: </span> {{ item.apr_money }}元</div>
      </div>
      <div style="width: 60%" class="right">
        <div class="desc"><span>投资时间: </span> {{ item.addtime_date }}</div>
        <div class="desc"><span>到期时间: </span> {{ item.end_time_date }}</div>
      </div>
      <div class="btn">
        <div class="button" style="float: left">
          <router-link class="bbtn" :to="'/touzi/apr/' + item['id']"
            >详情</router-link
          >
        </div>

        <div class="button" style="float: right">
          <router-link class="bbtn" :to="/contract/ + item['id']"
            >合同</router-link
          >
        </div>
      </div>
